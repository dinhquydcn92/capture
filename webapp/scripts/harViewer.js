        ...
        HarView.prototype = Lib.extend(new TabView(),
            /** @lends HarView */
            {
                ...
                appendPreview: function (jsonString) {
                    ...
                    var choiseType = $('#choiseTypeView').val();
                    $('.previewList').html('');
                    if (choiseCheck === 'url') {
                        $.ajax({
                            type: "POST",
                            url: "http://parser.quynd.com:4300/send/url",
                            data: {url: jsonString, type: choiseType},
                            success: function (data) {
                                if (data.data != 'fail') {
                                    if (data && data !== undefined) {
                                        try {
                                            var validate = $("#validate").prop("checked");
                                            data.data = data.data.replace(/Unsafe JavaScript attempt to access frame with URL about:blank from frame with URL file:\/\/netsniff\.js\. Domains, protocols and ports must match\./gi, '');

                                            var input = HarModel.parse(data.data);
                                            key.model.append(input);
                                        ...
                                        $('#myModal').modal('hide');
                                        //var linkImage = JSON.parse(data.data).log.link;
                                        //$('.harViewBodies').append('<div class="images-return"><img src="http://test.quynd.com/' + linkImage + '"></div>')
                        ...
                        $.ajax({
                            type: "POST",
                            url: "http://parser.quynd.com:4300/send/script",
                            data: {script: jsonString, type: choiseType},
                            success: function (data) {
                                    ...
                                    if (data && data !== undefined) {
                                        try {
                                            if (choiseType === 'all') {
                                                ...
                                                m.log.entries.splice(0, 1);
                                                var arr = JSON.stringify(m);
                                            } else {
                                                var arr = data.data;
                                            }
                                            var validate = $("#validate").prop("checked");
                                            var input = HarModel.parse(arr, validate);
                                            key.model.append(input);
